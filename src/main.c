...
static int main_scan(struct Masscan *masscan) {
  ...
  for (;;) {
    ...
    if (masscan->output.is_status_updates) {
      ...
      LOG(1,
          "[+] exiting receive thread: transmit_count > masscan->nic_count\n");
      is_tx_done = 1;
      is_rx_done = 1;
      ...
      LOG(1,
          "[+] exiting receive thread: receiev_count > masscan->nic_count\n");
    } else {
      /* [AFL-fuzz]
      ...
      LOG(1,
          "[+] exiting transmit thread: hit 'is_status_updates' else clause\n");
      is_tx_done = 1;
      is_rx_done = 1;
    }
