  ...
  for (i = start; i < end;) {
    ...
    while (batch_size && i < end) {
      ...
      if (xXx < range_ipv6) {
        ...
        /* flush queue on last packet in batch (only impacts pfring )
         * previously if provided a range
         * tldr the issue was related to the way masscan flushed packets in the
         * pfring transmit buffer. previously it required you to fill the entire
         * transmit ring buffer before it flushed the packets and sent them. the
         * change i made now explicitly flushes the buffer after each batch
         * instead of relying on pfring to do it for us when the buffer fills up
         */
        bool should_flush = batch_size == 1;
        ...
        rawsock_send_probe_ipv4(
            adapter, ip_them, port_them, ip_me, port_me, (unsigned)cookie,
            should_flush, /* flush queue on last packet in batch */
            &pkt_template);
      }
